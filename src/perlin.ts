...
function lerp(a: NoiseValue, b: NoiseValue, t: InterpolationFactor): NoiseValue {
  return a + t * (b - a);
}
...
function sampleNoise(coordinate: Coordinate, vectorGrid: GradientVectorGrid): NoiseValue {
...
function createNoiseArray(size: ImageSize, scale: ScaleFactor, vectorGrid: GradientVectorGrid): NoiseArray {
...
function createImageBuffer(imageSize: ImageSize, noiseArray: NoiseArray): ImageBuffer {
...
export function createPerlinNoise(imageSize: ImageSize, scaleFactor: ScaleFactor): ImageBuffer {
