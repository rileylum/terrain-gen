...
import { TerrainType, RGBAValue } from '../types';
...
describe('Terrain Functionality', () => {
  ...
  describe('TERRAIN_CONFIG', () => {
    ...
    it('should have continuous elevation coverage', () => {
      const sortedTerrains = Object.entries(TERRAIN_CONFIG).sort(
        (a, b) => a[1].maxElevation - b[1].maxElevation
      );

      expect(sortedTerrains[0][1].minElevation).toBe(-1.0);
      expect(sortedTerrains[sortedTerrains.length - 1][1].maxElevation).toBe(
        1.0
      );
  ...
  describe('Terrain Types', () => {
    it('should have correct terrain type definitions', () => {
      ...
      const sampleTerrain: TerrainType = {
        minElevation: -1.0,
        maxElevation: 0.0,
    ...
    it('should have correct RGBA value structure', () => {
      const sampleColor: RGBAValue = { R: 255, G: 128, B: 64, A: 255 };
