...
describe('Browser Module', () => {
  ...
  describe('Terrain Editor Functionality', () => {
    ...
    it('should handle tab switching', () => {
      ...
      const terrainTab = document.querySelector(
        '[data-tab="terrain-settings"]'
      ) as HTMLButtonElement;
      const imageTab = document.querySelector(
        '[data-tab="image-settings"]'
      ) as HTMLButtonElement;
      ...
      expect(terrainPanel?.classList.contains('active')).toBe(true);
      expect(imagePanel?.classList.contains('active')).toBe(false);
    });
    ...
    it('should reset to default terrains when reset button is clicked', () => {
      ...
      // Add a new terrain
      addBtn.click();
      const countAfterAdd = container?.children.length || 0;

      // Reset to default
      resetBtn.click();
      const countAfterReset = container?.children.length || 0;
    ...
    it('should handle elevation range calculations', () => {
      ...
      const container = document.getElementById('terrain-types-container');
      const terrainElements = container?.children;

      if (terrainElements && terrainElements.length > 0) {
        // Check that range indicators are present
        const firstTerrain = terrainElements[0];
        const rangeIndicator = firstTerrain.querySelector(
          '.elevation-range-indicator'
        );
        expect(rangeIndicator?.textContent).toContain('Range:');
        expect(rangeIndicator?.textContent).toContain('%');
      }
    });
    ...
    it('should handle terrain name changes', () => {
      ...
      if (nameInput) {
        ...
        const terrainElement = nameInput.closest(
          '.terrain-type'
        ) as HTMLElement;
